...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered abp address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ABP.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal abp:URI (abp:XyZ...?message=example)
            {
...
static void NotifyzABPReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzABPReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzABPReset.connect(boost::bind(NotifyzABPReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzABPReset.disconnect(boost::bind(NotifyzABPReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zabpTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
