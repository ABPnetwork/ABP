...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ABP);
    ...
    if (!settings.contains("nAnonymizeABPAmount"))
        settings.setValue("nAnonymizeABPAmount", 1000);

    nAnonymizeABPAmount = settings.value("nAnonymizeABPAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeABPAmount"))
        SoftSetArg("-anonymizeabpamount", settings.value("nAnonymizeABPAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in abp.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeABPAmount:
            return QVariant(nAnonymizeABPAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeABPAmount:
            nAnonymizeABPAmount = value.toInt();
            settings.setValue("nAnonymizeABPAmount", nAnonymizeABPAmount);
            emit anonymizeABPAmountChanged(nAnonymizeABPAmount);
            break;
        case CoinControlFeatures:
