...
public:
    ...
    bool WriteZABPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZABPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZABPSeed_deprecated(uint256& seed);
    bool EraseZABPSeed();
    bool EraseZABPSeed_deprecated();

    bool WriteZABPCount(const uint32_t& nCount);
    bool ReadZABPCount(uint32_t& nCount);
