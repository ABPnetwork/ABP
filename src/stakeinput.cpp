...
// The zABP block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zABP is a hash of the serial
...
{
    //Create an output returning the zABP that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZABPOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zABP output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zABP", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZABPOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zABP output", __func__);
...
{
    CzABPTracker* zabpTracker = pwallet->zabpTracker.get();
    CMintMeta meta;
    if (!zabpTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zabpTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a ABP stake is the outpoint
