...
#if defined(NDEBUG)
#error "ABP cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zABP is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zABP spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zABP without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zABP spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zABP spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zABP spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zabp");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zABP mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight <= 10080 && nHeight >= 2) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <=20160  && nHeight >=10081 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=30240  && nHeight >=20161 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=40320  && nHeight >=30241 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=50400  && nHeight >=40321 ) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <=60480  && nHeight >=50401 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=70560  && nHeight >=60481 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=80640  && nHeight >=70561 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=90720  && nHeight >=80641 ) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <=100800  && nHeight >=90721 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=110880  && nHeight >=100801 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=120960  && nHeight >=110881 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=131040  && nHeight >=120961 ) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <=141120  && nHeight >=131041 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=151200  && nHeight >=141121 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=161280  && nHeight >=151201 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=171360  && nHeight >=161281 ) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <=181440  && nHeight >=171361 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=191520  && nHeight >=181441 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=201600  && nHeight >=191521 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=211680  && nHeight >=201601 ) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <=221760  && nHeight >=211681 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=231840  && nHeight >=221761 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=241920  && nHeight >=231841 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=252000  && nHeight >=241921 ) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <=262080  && nHeight >=252001 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=272160  && nHeight >=262081 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=282240  && nHeight >=272161 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=292320  && nHeight >=282241 ) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <=302400  && nHeight >=292321 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=312480  && nHeight >=302401 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=322560  && nHeight >=312481 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=332640  && nHeight >=322561 ) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <=342720  && nHeight >=332641 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=352800  && nHeight >=342721 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=362880  && nHeight >=352801 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=372960  && nHeight >=362881 ) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <=383040  && nHeight >=372961 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=393120  && nHeight >=383041 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=403200  && nHeight >=393121 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=413280  && nHeight >=403201 ) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <=423360  && nHeight >=413281 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=433440  && nHeight >=423361 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=443520  && nHeight >=433441 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=453600  && nHeight >=443521 ) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <=463680  && nHeight >=453601 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=473760  && nHeight >=463681 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=483840  && nHeight >=473761 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=493920  && nHeight >=483841 ) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <=504000  && nHeight >=493921 ) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=514080  && nHeight >=504001 ) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <=524160  && nHeight >=514081 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=1049760  && nHeight >=524160 ) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <=1575360  && nHeight >=1049761 ) {
        nSubsidy = 8 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZABPStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from ABP
...
{
    RenameThread("abp-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZABPMinted()
{
...
void RecalculateZABPSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zABP supply
...
bool RecalculateABPSupply(int nHeightStart)
{
...
{
    // ABP: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZABPSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZABPMinted();
        RecalculateZABPSpent();
        RecalculateABPSupply(Params().Zerocoin_StartHeight());
    }

    //Track zABP money supply in the block index
    if (!UpdateZABPSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zABP supply for block=%s height=%d", __func__,
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    if (pindex->pprev->nHeight >= 2) {
      ...
      if (devReward > 0) {
        ...
        if (!DevPaid) {
            return state.DoS(0, error("ConnectBlock(ABP): no dev reward"), REJECT_INVALID, "no-dev-reward");
        }
      }

      CAmount govReward = nExpectedMint * .15;
      if (govReward > 0) {
        ...
        for (const auto& output : block.vtx[(block.IsProofOfWork()?0:1)].vout) {
            if (output.scriptPubKey == GOV_SCRIPT && output.nValue == govReward) {
                GovPaid = true;
                break;
            }
        }
        if (!GovPaid) {
            return state.DoS(0, error("ConnectBlock(ABP): no gov reward"), REJECT_INVALID, "no-gov-reward");
        }
      }
    }
    // ABP>
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ABP to zABP
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zABP spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zABP serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zABP stake block is less than allowed start height", __func__);

    if (CZPivStake* zABP = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zABP->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zABP stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zABP stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zABP->GetValue()));
        if (nChecksum200 != zABP->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zABP->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZABP() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zABP fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zABP mints and %d zABP spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ABP: We use certain sporks during IBD, so check to see if they are
