...
#ifndef ABP_ZABPWALLET_H
#define ABP_ZABPWALLET_H
...
public:
    CzABPWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZABP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZABP(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ABP_ZABPWALLET_H
