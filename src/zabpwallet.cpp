...
CzABPWallet::CzABPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zabp seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZABPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZABPSeed_deprecated()) {
                    LogPrintf("%s: Updated zABP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zabp seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zabp wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzABPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZABPCount(nCountLastUsed);
    else if (!walletdb.ReadZABPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzABPWallet::Lock()
{
    seedMaster = 0;
}

void CzABPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzABPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZABP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzABPWallet::LoadMintPoolFromDB()
{
...
void CzABPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzABPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzABPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zabpTracker->HasPubcoinHash(pMint.first)) {
...
bool CzABPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZABP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zabpTracker which also adds to database
    pwalletMain->zabpTracker->Add(dMint, true);
...
void CzABPWallet::SeedToZABP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzABPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzABPWallet::GenerateDeterministicZABP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzABPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzABPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
